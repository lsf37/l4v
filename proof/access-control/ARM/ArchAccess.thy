...
fun cap_asid' :: "cap \<Rightarrow> asid set" where
  "cap_asid' (ArchObjectCap (PageCap _ _ _ _ mapping)) = fst ` set_option mapping"
| "cap_asid' (ArchObjectCap (PageTableCap _ mapping)) = fst ` set_option mapping"
| "cap_asid' (ArchObjectCap (PageDirectoryCap _ asid_opt)) = set_option asid_opt"
...
inductive_set state_asids_to_policy_aux for aag caps asid_tab vrefs where
  sata_asid:
    "\<lbrakk> caps ptr = Some cap; asid \<in> cap_asid' cap \<rbrakk>
...
abbreviation
  "state_asids_to_policy aag s \<equiv>
     state_asids_to_policy_aux aag (caps_of_state s) (asid_table s) (state_vrefs s)"
