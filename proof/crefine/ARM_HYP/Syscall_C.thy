...
lemma getMessageInfo_ccorres:
  "ccorres (\<lambda>rv rv'. rv = messageInfoFromWord rv') ret__unsigned_long_' \<top>
       (UNIV \<inter> {s. thread_' s = tcb_ptr_to_ctcb_ptr thread}
             \<inter> {s. reg_' s = register_from_H ARM_HYP_H.msgInfoRegister}) []
...
lemma handleInvocation_def2:
  "handleInvocation isCall isBlocking =
   do thread \<leftarrow> getCurThread;
      ...
      ptr \<leftarrow> asUser thread (getRegister ARM_HYP_H.capRegister);
...
lemma handleInvocation_ccorres:
  ...
  apply (cinit' lift: isCall_' isBlocking_'
   ...
   apply (simp add: Kernel_C.msgInfoRegister_def ARM_HYP_H.msgInfoRegister_def
                    ARM_HYP.msgInfoRegister_def Kernel_C.R1_def
                    Kernel_C.capRegister_def ARM_HYP_H.capRegister_def
                    ARM_HYP.capRegister_def Kernel_C.R0_def)
...
lemma handleRecv_ccorres:
  ...
  apply (simp add: Kernel_C.capRegister_def ARM_HYP_H.capRegister_def ct_in_state'_def
                   ARM_HYP.capRegister_def Kernel_C.R0_def
                   tcb_at_invs')
...
lemma vgicUpdateLR_ccorres_armHSCurVCPU:
  ...
  apply (rule ccorres_guard_imp)
    ...
    apply (rule_tac P="ko_at' vcpu vcpuptr"
     ...
     apply (clarsimp simp add: objBits_simps archObjSize_def machine_bits_defs)+
...
definition
  eisr_calc :: "machine_word \<Rightarrow> machine_word \<Rightarrow> nat"
where
      ...
      apply (clarsimp simp: cfault_rel_def seL4_Fault_VGICMaintenance_lift_def
          ...
          apply (ctac add: get_gic_vcpu_ctrl_lr_ccorres)
            ...
            apply (rule_tac xf'=virq_'
                     and val="virq_C (ARRAY _. ARM_HYP_H.virqSetEOIIRQEN virq 0)"
  ...
  show ?thesis
    ...
    subgoal for _ _ eisr0 eisr1
      ...
      apply (clarsimp simp: cfault_rel_def seL4_Fault_VPPIEvent_lift_def
                            ...
                            mask_eq_iff
                      split: if_splits)
      apply (simp add: ucast_and_mask ge_mask_eq)
      done
